...
Insomnia should stop treating lid-close locking as an immediate custom security action. When Insomnia keeps the Mac awake with the lid closed, it should mirror what macOS would have done if native display sleep were allowed to complete.

The behavior should be simple and predictable:

- If macOS **Require password** is **Never**, Insomnia does not lock on lid close.
- If macOS would require a password, Insomnia waits for the effective macOS timing before locking: the current power source's display-off timer plus the Require Password delay.
- If the lid opens before that delay completes, Insomnia cancels the pending lock.

This keeps Insomnia layered on top of macOS settings instead of creating a separate lock policy.
...
- Preserve the existing computed lock matrix: Insomnia only fills the lid-close lock gap when it is active, lid-close sleep prevention is enabled, and display sleep prevention is off.
...
- Do not add a new user-facing Insomnia lock setting.
- Do not change Insomnia's display-sleep or lid-close sleep-prevention preferences.
...
- Do not block app shutdown, Insomnia off, or lid reopening on a pending lock timer.

## User-Facing Behavior

When Insomnia is off, nothing changes. macOS owns all sleep, display, screensaver, and lock behavior.

When Insomnia is on but **Prevent system sleep with lid closed** is off, closing the lid follows native macOS behavior. Insomnia does not run a lid-close lock timer.

When Insomnia is on, **Prevent system sleep with lid closed** is on, and **Prevent display sleep** is on, Insomnia still does not lock on lid close. Display sleep is being intentionally suppressed, so the native idle display/screen saver trigger is also intentionally suppressed.

When Insomnia is on, **Prevent system sleep with lid closed** is on, and **Prevent display sleep** is off:

1. Closing the lid reads the current macOS lock/display policy.
2. If Require Password is Never, Insomnia leaves the session unlocked.
3. If Require Password is enabled, Insomnia schedules a lock for:
...
5. If the scheduled time arrives while Insomnia is still active and the lid is still closed under the same gating conditions, Insomnia locks the screen.
...
If Require Password were set to 5 seconds instead of Immediately, Insomnia would lock after 1 minute 5 seconds on battery and 5 minutes 5 seconds on AC power.

## Architecture

### `MacOSLockPolicyReader`

Add a focused reader in `InsomniaCore` that returns a snapshot of the macOS policy needed by lid-close locking.
...
If the timer cannot be read, fail closed for surprise-minimization by not scheduling a Insomnia lock and log the policy-read failure. Do not lock immediately as a fallback.
...
Preference changes should not require a separate observer for this first version. Rechecking at fire time is enough for safety: turning display prevention on, turning lid-close prevention off, or turning Insomnia off prevents the delayed lock from firing. Lid reopen cancels promptly from the lid event.
...
- Do not change Insomnia active/inactive state when a lock cannot be scheduled or executed.
...
- Turning Insomnia inactive before the delay completes prevents locking.
...
1. Set Require Password to Never, turn Insomnia on, enable lid-close prevention, allow display sleep, close and reopen after longer than the display-off timer: session is not locked by Insomnia.
...
7. Close lid with a pending lock, turn Insomnia off before the delay if possible through external display or remote interaction: pending lock does not fire.

## Documentation Updates

Update `README.md` to say:

- Insomnia does not expose its own lock setting.
- When lid-close sleep prevention keeps the Mac awake and display sleep is allowed, Insomnia mirrors macOS lock policy.
- Require Password = Never means Insomnia will not lock on lid close.
- Otherwise, Insomnia locks after the active display-off timer plus the Require Password delay.
...
- **New Insomnia lock delay setting:** rejected because the goal is to make Insomnia self-explanatory by following macOS, not by creating another policy.
